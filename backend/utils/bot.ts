...
import {ClientEvent, EventType, MatrixEvent, MsgType, RelationType, RoomEvent, RoomMemberEvent, Visibility} from "matrix-js-sdk";
import olm from "olm";
...
function parseMessageToSelf(event: MatrixEvent) {
...
function sayGoodbyIfNecessary(event: MatrixEvent) {
...
function createRoomAndInvite(roomName: string, userMailList: string[]) {
    let roomId
...
client.on(RoomEvent.Timeline, function (event, _room, _toStartOfTimeline) {
    ...
    if (event.getType() === EventType.RoomMessage) {
        ...
        if (isNewMessage) {
            ...
            if (roomId && [gmcdInfra].indexOf(roomId) > -1) {
...
function addEmoji(event: MatrixEvent, emoji: string) {
