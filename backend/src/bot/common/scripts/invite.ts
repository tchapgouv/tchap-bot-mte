...
export function inviteInRoomIfAsked(client: MatrixClient, roomId: string, userId: string, body: string) {
    ...
    if (roomId && body && inviteInRoomOptions.some(option => body.includes(option))) {
        ...
        // Bot-Gmcd [Developpement-Durable] invite @quentin.rabier-developpement-durable.gouv.fr:agent.dev-durable.tchap.gouv.fr !GCDRinCBfdhIEpoecb:agent.dev-durable.tchap.gouv.fr
        ...
        if (isSupport(userId)) {
            ...
            botService.getRoomName(roomToInvite).catch(() => {
