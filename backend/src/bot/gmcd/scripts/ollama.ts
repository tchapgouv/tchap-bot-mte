 ...
 * CONTEXT=$(curl -k https://Ollama.gmcd-runner01.eco4.cloud.e2.rie.gouv.fr/api/generate -d '{ "model": "llama3", "prompt": "Bonjour. Ce qui suit est le contexte te permettant de répondre aux futures questions : Répond toujours en français. Ton nom est Bot-GMCD. Lis la documentation suivante (https://fabrique-numerique.gitbook.io/bnum) en suivant les liens des pages, récursivement, tant qu''ils contiennent la racine : fabrique-numerique.gitbook.io/bnum. Répond à mes questions en utilisant cette documentation. Ne parle du bureau numérique que si la question a trait à la messagerie et qu''une réponse existe dans la documentation. Ne précise pas que tu lis la documentation dans tes réponses.", "stream": false }' | jq .context) && curl  -k https://Ollama.gmcd-runner01.eco4.cloud.e2.rie.gouv.fr/api/generate -d '{ "model": "llama3", "prompt": "Bonjour.", "context":'$CONTEXT', "stream": false }' | jq .response &&  curl  -k https://Ollama.gmcd-runner01.eco4.cloud.e2.rie.gouv.fr/api/generate -d '{ "model": "llama3", "prompt": "Bonjour. Pourquoi le ciel est bleu ?", "context":'$CONTEXT', "stream": false }' | jq .response &&  curl  -k https://Ollama.gmcd-runner01.eco4.cloud.e2.rie.gouv.fr/api/generate -d '{ "model": "llama3", "prompt": "Bonjour. Comment changer son mot de passe ?", "context":'$CONTEXT', "stream": false }' | jq .response
 */
