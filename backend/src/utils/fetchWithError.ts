...
export default function (url: string, opts: { requestInit?: RequestInit, timeout?: number, proxify?: boolean } = {}): Promise<any> {

    if (!opts.requestInit) opts.requestInit = {method: "GET"}
    if (!opts.timeout) opts.timeout = 7000
    if (!opts.proxify) opts.proxify = false

    if (opts.proxify) opts.requestInit.agent = process.env.PROXY_URL ? new HttpsProxyAgent(process.env.PROXY_URL) : undefined
